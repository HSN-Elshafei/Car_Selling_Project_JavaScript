...
const updateProductQuantity = async (user, productId, newQuantity) => {
  ...
  try {
    ...
    const productElement = document.querySelector(`.basket-product[data-id="${productId}"]`);
    const price = parseFloat(productElement.querySelector(".price").textContent);
    const newSubtotal = price * newQuantity;
    productElement.querySelector(".subtotal").textContent = newSubtotal.toFixed(2);
...
document.querySelector(".checkout-cta").addEventListener("click", async () => {
